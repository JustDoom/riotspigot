...
public class MojangsonParser {
    ...
    static MojangsonTypeParser a(String... astring) throws MojangsonParseException {
        return a(astring[0], astring[1]);
    }

    static MojangsonTypeParser a(String s, String s1) throws MojangsonParseException {
        s1 = s1.trim();
        String s2;
        boolean flag;
        char c0;
        ...
        if (s1.startsWith("{")) {
            ...
            MojangsonCompoundParser mojangsonparser_mojangsoncompoundparser;

            for (mojangsonparser_mojangsoncompoundparser = new MojangsonCompoundParser(s); s1.length() > 0; s1 = s1.substring(s2.length() + 1)) {
                s2 = b(s1, true);
                if (s2.length() > 0) {
                    flag = false;
            ...
            MojangsonListParser mojangsonparser_mojangsonlistparser;

            for (mojangsonparser_mojangsonlistparser = new MojangsonListParser(s); s1.length() > 0; s1 = s1.substring(s2.length() + 1)) {
                s2 = b(s1, false);
                if (s2.length() > 0) {
                    flag = true;
    ...
    private static MojangsonTypeParser a(String s, boolean flag) throws MojangsonParseException {
        String s1 = c(s, flag);
        String s2 = d(s, flag);
    ...
    static class MojangsonPrimitiveParser extends MojangsonTypeParser {
    ...
    static class MojangsonListParser extends MojangsonTypeParser {

        protected List<MojangsonTypeParser> b = Lists.newArrayList();
        ...
        public NBTBase a() throws MojangsonParseException {
            ...
            while (iterator.hasNext()) {
                MojangsonTypeParser mojangsonparser_mojangsontypeparser = (MojangsonTypeParser) iterator.next();
    ...
    static class MojangsonCompoundParser extends MojangsonTypeParser {
