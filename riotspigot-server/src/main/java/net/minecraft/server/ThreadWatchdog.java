...
public class ThreadWatchdog implements Runnable {
    ...
    public void run() {
        while (this.b.isRunning()) {
            ...
            if (k > this.c) {
                ThreadWatchdog.a.fatal("A single server tick took " + String.format("%.2f", (float) k / 1000.0F) + " seconds (should be max " + String.format("%.2f", new Object[] { Float.valueOf(0.05F)}) + ")");
