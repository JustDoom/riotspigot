...
public class SimpleHelpMap implements HelpMap {
    ...
    //private HelpYamlReader yaml; Dytanic edit remove help.yml configuration file
    ...
    public SimpleHelpMap(CraftServer server) {
        ...
        this.server = server;
        /* Dytanic edit remove help.yml configuration file
    ...
    public synchronized HelpTopic getHelpTopic(String topicName) {
        if (topicName.equals($.EMPTY_STRING)) {
            return defaultTopic;
        }
    ...
    public List<String> getIgnoredPlugins() {
        return /*yaml.getIgnoredPlugins()*/ Collections.emptyList(); //Dytanic edit replace with empty List
    }

    /**
     * Reads the general topics from help.yml and adds them to the help index.
     */
    //Dytanic edit remove help.yml configuration support
    ...
    public synchronized void initializeCommands() {
        ...
        Set<String> ignoredPlugins = new HashSet<String>(/*yaml.getIgnoredPlugins()*/ getIgnoredPlugins()); //Dytanic edit replace with existing local method
        ...
        for (Command command : server.getCommandMap().getCommands()) {
            ...
            for (String alias : command.getAliases()) {
                ...
                if (server.getCommandMap().getCommand(alias) == command) {
                    addTopic(new CommandAliasHelpTopic($.SLASH_STRING + alias, $.SLASH_STRING + command.getLabel(), this));
                }
            }
        }
        ...
        /* Dytanic edit remove help.yml configuration file support
    ...
    private void fillPluginIndexes(Map<String, Set<HelpTopic>> pluginIndexes, Collection<? extends Command> commands) {
        for (Command command : commands) {
            ...
            if (pluginName != null) {
                HelpTopic topic = getHelpTopic($.SLASH_STRING + command.getLabel());
                if (topic != null) {
