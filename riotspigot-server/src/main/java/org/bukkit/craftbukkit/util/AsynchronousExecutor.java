...
public final class AsynchronousExecutor<P, T, C, E extends Throwable> {
    ...
    //final ThreadPoolExecutor pool; Dytanic edit
    ...
    public AsynchronousExecutor(final CallBackProvider<P, T, C, E> provider, final int coreSize) {
        ...
        //pool = new ThreadPoolExecutor(coreSize, Integer.MAX_VALUE, 60l, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), provider); Dytanic edit remove ThreadPoolExecutor
    }
    ...
    public void add(P parameter, C callback) {
        ...
        if (task == null) {
            ...
            //pool.execute(task);
            DytanicSpigot.getInstance().getTaskScheduler().schedule(task);
        }
    ...
    public void setActiveThreads(final int coreSize) {
        //pool.setCorePoolSize(coreSize); Using TaskScheduler from Dytanic
    }
}
